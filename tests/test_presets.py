...
def test_eckardt_and_petiziol_in_every_preset():
    for name in preset_names():
        authors = [a.lower() for a in preset_config(name).named_authors]
        assert "eckardt" in authors, name
        assert "petiziol" in authors, name
...
def test_merge_preset_unions_without_touching_scalars():
    ...
    assert "eckardt" in [a.lower() for a in merged.named_authors]
...
def test_merge_preset_dedups_case_insensitively_preserving_order():
    base = Config(core_keywords=["Floquet", "unique-base-kw"], named_authors=["Eckardt"],
                  feeds={}, default_feeds=[])
    ...
    assert [a.lower() for a in merged.named_authors].count("eckardt") == 1
